...
// Unit Test for testing TestBed using a solar system.
// Tests the basic mat4 TestBed, such as scaling, rotation, and translation.
...
public:
	...
	bool Load()
	{
		if (mSettings.mResetGraphics || !pRenderer)
		{
			...
			ResourceLoaderDesc resouceLoaderDesc = gDefaultResourceLoaderDesc;
			resouceLoaderDesc.mSingleThreaded = true;
			initResourceLoaderInterface(pRenderer, &resouceLoaderDesc);
	...
	void Draw()
	{
		...
		*(UniformBlock*)skyboxViewProjCbv.pMappedData = gUniformDataSky;
		...
		if (gTakeScreenshot)
		{
			...
			{
				captureScreenshot(pSwapChain, swapchainImageIndex, RESOURCE_STATE_PRESENT, "00_TestBed_Screenshot.png");
	...
	const char* GetName() { return "00_TestBed"; }
...
DEFINE_APPLICATION_MAIN(TestBed)
